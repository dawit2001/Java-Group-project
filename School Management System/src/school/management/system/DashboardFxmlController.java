...
public class DashboardFxmlController implements Initializable {
    ...
    public void showTeacherData() {                        //  this used to map the column and the object property and to dispaly it to the tableview
      ...
      try { teacherDataList = teacherDataList();
...
//            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd"); // set the date format to match the format used in SQLite
    ...
    public void studentsAddBtn() throws SQLException {      // this add button used for inserting data to the database from the from
        ...
        if (students_studentID.getText().isEmpty() || students_name.getText().isEmpty()
            ...
            if (result.next()) {
                errorMessage("Student ID: " + students_studentID.getText() + " was already taken!");
    ...
    public void studentsUpdateBtn() throws SQLException {
        ...
        if (students_name.getText().isEmpty()
            ...
            if (alert.showAndWait().get() == ButtonType.OK) {
                ...
                prepare.setString(3, students_department.getText());                               // can also use String.valueOf in place of casting but not the reverse in most case
                // prepare.setString(4, students_phoneNum.getText());                              //2. the JDBC driver will automatically convert the String value to an int before storing it in the database.so we can set it like this for any data type
                prepare.setInt(4, Integer.parseInt(students_phoneNum.getText()));          //but using the appropriate method is good for handing error in this case better to use parepare.setiInt instead of setString
                prepare.setString(5, students_address.getText());
                prepare.setInt(6, (int) students_year.getSelectionModel().getSelectedItem());   //3. this getSelectedItem() returns object of type Integer so we can cast instead of use Integer.parseInt() since it expect string but the return value is not string
                prepare.setString(7, String.valueOf(students_startSemister.getValue()));       //4. String.valueOf() can convert any Data type to string // in this case only use this instead of casting since casting works for few data type like numerics
    ...
    public void paymentShowSpecificData() {
        ...
        if (found) {
            ...
            errorMessage("There is no unpaid Student with this ID:" + payment_studentID.getText());
        }
    ...
    public void paymentAmount() {
        ...
        if (payment_studentID.getText().isEmpty() || payment_amount.getText().isEmpty()) {
            ...
            errorMessage("There is somthing worng");
    ...
    public void paymentPayBtn(ActionEvent event) throws SQLException {
        ...
        if (totalP == 0 || amount == 0 || payment_amount.getText().isEmpty()) {
            ...
            errorMessage("Something Wrong :3");
    ...
    public void logout() {
        try {
            ...
            if (option.isPresent() && option.get().equals(ButtonType.OK)) {    // it used to check weather user clike the button(Ok OR Cancel) or not(X) close the dialog box 
                ...
                fwr.write("Admin Name: " + Data.username + " logged Outinto: " + currentDateTime + "\n");
                fwr.close();
            }
