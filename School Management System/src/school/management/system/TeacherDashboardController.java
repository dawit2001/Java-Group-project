...
public class TeacherDashboardController implements Initializable {
    ...
    public void addMarkList() {
        ...
        if (studentID_tf.getText().isEmpty() || studentName_tf.getText().isEmpty()
            ...
            errorMessage("Please fill all blank fields"); // if one of them is null or empty it calles the emptyFields
                                                          // method
            ...
            try {
                ...
                if (result.next()) {
                    ...
                    String checkRepition = "SELECT courseID FROM grade WHERE studentID = '" + studentID_tf.getText()
                            + "' AND courseID = '" + courseID_tf.getText() + "'";
                    statement = connect.createStatement();
                    result = statement.executeQuery(checkRepition);

                    if (result.next()) {
                        errorMessage("This CourseID: " + courseID_tf.getText() + " Is Aleardy Filed For StudnetID: "
                                + studentID_tf.getText());
    ...
    private void markListSelect(MouseEvent event) {
        ...
        if ((num - 1) < -1) { // if there no is selected row i.e index < 0 it simply return wihtout doing
                              // anything
            return;
        }
    ...
    public void logout() {
        try {
            ...
            Optional<ButtonType> option = alert.showAndWait(); // this Optional<ButtonType> java class uesed as a
                                                               // container object may or mayn't have null value

            if (option.isPresent() && option.get().equals(ButtonType.OK)) { // it used to check weather user clike the
                                                                            // button(Ok OR Cancel) or not(X) close the
                                                                            // dialog box
